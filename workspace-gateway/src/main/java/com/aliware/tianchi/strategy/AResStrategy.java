...
public class AResStrategy extends AbstractStrategy {
    ...
    public int select(URL url, Invocation invocation) {
        ...
        double k1 = Math.log(rand.nextDouble()) / (smallActiveCount * Constants.smallProducerThreadSum);
        double k2 = Math.log(rand.nextDouble()) / (mediumActiveCount * Constants.mediumProducerThreadSum);
        double k3 = Math.log(rand.nextDouble()) / (largeActiveCount * Constants.largeProducerThreadSum);
